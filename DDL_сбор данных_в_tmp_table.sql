...
--Добавляем данные из всех источниковe во временную таблицу весь скрипт выполняем в рамках одной транзакции
BEGIN TRANSACTION
--select * from dwh.d_customer dc ;
...
union 									--добавление данных из external_source
 select 
 		t1.order_id,
        t1.order_created_date,
        t1.order_completion_date,
...
WHEN NOT MATCHED THEN
 ...
 select * from dwh.d_craftsman dc 			--проверочный мастер из external_source
where craftsman_name = 'Kerrill Watman' ;
COMMIT TRANSACTION
